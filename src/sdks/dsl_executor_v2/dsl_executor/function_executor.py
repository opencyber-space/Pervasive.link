...
class LocalCodeExecutor:
    def __init__(self, download_url: str, global_settings: dict, global_parameters: dict, settings: dict, parameters: dict, global_state: dict, addons_manager=None, inference_service=None):
    ...
    def invoke_addon(self, addon_type: str, addon_name: str, data: dict):
        if not self.addons_manager:
            raise RuntimeError("AddonsManager not configured in LocalCodeExecutor")

        if addon_type == "webhook":
            return self.addons_manager.invoke_webhook(addon_name, data)
        elif addon_type == "callback":
            return self.addons_manager.invoke_callback(addon_name, data)
        elif addon_type == "inference":
            if not self.inference_service:
                raise RuntimeError("InferenceService not configured in LocalCodeExecutor")
            # Using synchronous inference call via InferenceService wrapper
            return self.inference_service.run_inference(addon_name, data)
        else:
